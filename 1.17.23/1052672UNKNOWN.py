        ...
        for base_size in range(2, 257):
            for alphabet_multiplier in range(1, 10):
                for base_multiplier in range(1, 10):
                    encoded = optimize_passes(encode_baseX(input_string, alphabet_size, base_size, alphabet_multiplier, base_multiplier))
...
def optimize_passes(input_string):
    prev_encoded = input_string
    encoded = encode_baseX(input_string)
    while len(encoded) < len(prev_encoded):
        prev_encoded = encoded
        encoded = encode_baseX(encoded)
    return encoded