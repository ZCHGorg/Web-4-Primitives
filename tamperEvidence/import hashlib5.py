...
def encrypt_transaction_data(data, key):
    try:
        ...
        # Concatenate the encrypted data and IV as a colon-
        # separated string and return the result
        return f"{encrypted_data_b64}:{iv_b64}"

    except Exception as e:
        # Print an error message and return None if an exception is raised
        print(f"An error occurred while encrypting the transaction data: {e}")
        return None
...
def decrypt_transaction_data(encrypted_message, key):
    try:
        ...
        # Print an error message and return None if an exception is raised
        print(f"An error occurred while decrypting the transaction data: {e}")
        return None

def encrypt_key(key, public_keys):
    # Encrypt the key using the public keys of the receiving nodes
    encrypted_keys = {}
    for node_id, public_key in public_keys.items():
        cipher = PKCS1_OAEP.new(public_key)
        encrypted_key = cipher.encrypt(key)
        encrypted_keys[node_id] = encrypted_key

    # Encode the encrypted keys as a JSON object and return the result
    return json.dumps(encrypted_keys)

def decrypt_key(encrypted_key, private_key):
    # Decode the encrypted key data from a JSON object
    encrypted_keys = json.loads(encrypted_key)
